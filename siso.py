...
def main():
    ...
    if args.extract == 'users':
        ...
        # Extraction complète: utilisateurs, groupes et membres dans un seul fichier
        output_file=f"{dirtarget}/Export_AD_Complete_{args.ad}_{datetime.now().strftime('%Y-%m-%d')}.xlsx"
        ad_helper.get_all_data(args.ad, output_file)
        if args.mail:
            mail_subject=f"Export complet AD {args.ad} du {datetime.now().strftime('%d/%m/%Y')}"
            mail_body=f"Bonjour,\n\nCi-joint un export complet (utilisateurs, groupes et membres) de l'AD {args.ad}.\n\nCe message est automatique."
